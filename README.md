...
**Each section's questions must use their own Word list** — do not continue the same list across a section heading into the next section. The script finds the boundary between adjacent sections by looking for a change in list identity in the underlying XML. If two sections share one continuous list, the boundary between them will not be detected and questions from both sections will be shuffled together.

**Continuous numbering across sections** (e.g. True/False 1–5, Multiple Choice 6–20) is supported — but must be achieved by setting the list start value in Word's numbering settings, not by manually typing numbers. The script copies list definitions unchanged, so after shuffling the questions within each section are re-counted from the same start value and the displayed numbering remains correct.
